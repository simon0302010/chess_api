...
use crate::{SharedApiState, pieces::{FromSimplifiedMove, SimplifiedMove}};
...
pub async fn make_move(
    ...
    let board_move = match BoardMove::from_simplified(payload.clone()) {
        ...
        Err(e) => return (
            ...
            Json(MakeMoveResponse { success: false, text: format!("Failed to parse source or destination: {}", e), next_side: state.board.get_side_to_move().to_string() })
        )
    };
    ...
    match state.board.make_move_mut(&board_move) {
        Ok(_) => {
            let piece_move = match board_move.piece_move() {
                Ok(mov) => mov,
                Err(e) => return (StatusCode::INTERNAL_SERVER_ERROR, Json(MakeMoveResponse {
                    success: false,
            ...
            (
                StatusCode::OK,
                Json(MakeMoveResponse { success: true, text: format!("Successfully moved {} from {} to {}", payload.piece_type, piece_move.get_source_square().to_string().to_uppercase(), piece_move.get_destination_square().to_string().to_uppercase()), next_side: state.board.get_side_to_move().to_string() })
            )
        },
        Err(e) => (
            StatusCode::UNPROCESSABLE_ENTITY,
            Json(MakeMoveResponse { success: false, text: format!("{}", e), next_side: state.board.get_side_to_move().to_string() })
        )
    }
}
